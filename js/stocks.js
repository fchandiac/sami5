...
const add_critical_imput = document.getElementById('add_critical_imput')
  ...
  if (search_product_stock_form.checkValidity()) {
    products.find_one_by_name(input_s.value).then(res => {
      if (res == null) {
        ...
        stocks_promises.create(res.id, add_room_input.value, add_warehouse_imput.value, add_critical_imput.value).
          then(res => {
            stock_grid.render()
          })
          .catch(err => {
...
function load_critical_alerts(){
  ...
  stocks_promises.alert_critical()
    .then(res => {
...
module.exports = { subtract_stock, load_critical_alerts }