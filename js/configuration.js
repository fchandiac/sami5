...
const select_orders_switch = document.getElementById('select_orders_switch')
...
if (config.orders == true){
    select_orders_switch.checked = true
} else {
    select_orders_switch.checked = false
}
...
save_config_btn.addEventListener('click', () => {
    ...
    if (select_orders_switch.checked == true){
        config_json.orders = true
    } else {
        config_json.orders = false
    }
