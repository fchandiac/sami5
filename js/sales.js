...
const sales_products_grid = require('./grids/sale_product_grid')
...
function render_sale_product_grid(start_date, end_date) {
    var end = moment(end_date).format('yyyy-MM-DD')
    var start = moment(start_date).format('yyyy-MM-DD')
    sales_detail.find_all_count_by_date_range_group_by_product(start, end)
    .then(res => {
        sales_products_grid.render(res, start.toString(), end.toString())
    })
    .catch(err => {err})
