                        ...
                        .then(res => {
                            ...
                            orders_to_close.forEach(item => {
                                ...
                                order_btn.addEventListener('click', () => {
                                    let index
                                    orders_to_close.forEach(item => {
                                        if (order_btn.id == item) {
                                            index = orders_to_close.indexOf(item)
                                        }
                                    })
                                    orders_to_close.splice(index, 1)
                                    order_btn.remove()
            ...
            order.forEach(order_item => {
                cart.forEach(cart_item => {
                    if (cart_item.id == order_item.product_id) {
                        let new_quanty = cart_item.quanty - order_item.quanty
                        if (new_quanty >= 1) {
                            cart_item.quanty = new_quanty
                        } else if (new_quanty == 0) {
                            let index = cart.indexOf(cart_item)
...
function create_order() {
    ...
    orders_promises.create().then(order => {
        //console.log(order)
        let promises_list = []
        cart.map(item => {
        ...
        Promise.all(promises_list)
            .then(() => {
                ...
                if (print_document_type != 2) {
                    ...
                    //print_order_by_config(order.id)
                } else {
                    cart = []
                    total = 0
                    total_label.dispatchEvent(new Event('input'))
            ...
            .then(order_detail => {
                ...
                if (order_to_print.length >= 1) {
                    if (config.second_ticket_printer == 1) {
                        ...
                        second_printer.print_order(order_to_print, order_id)
...
module.exports = { cart, add_item_to_cart, order_to_close_container, orders_to_close, on_delete_order }