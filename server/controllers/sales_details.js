...
async function find_all_count_by_date_range_and_product_group_by_price(start_date, end_date, product_id) {
    const sale = await SalesDetails.findAll({

        attributes: [
            'product_id',
            'price_id',
            [sequelize.fn('sum', sequelize.col('quanty')), 'quanty_sum'],
        ],
        ...
        //order: [[sequelize.col('quanty_sum'),'DESC']]

    }).then(data => { return { 'code': 1, 'data': data } }).catch(err => { return { 'code': 0, 'data': err } })
    return sale
}

async function find_all_count_by_date_range_group_by_product(start_date, end_date){
    const sale = await SalesDetails.findAll({
        ...
        attributes: [
            ...
            // 'price_id',subtotal
            [sequelize.fn('sum', sequelize.col('quanty')), 'quanty_sum'],
            [sequelize.fn('sum', sequelize.col('subtotal')), 'subtotal_sum'],
        ],
...
sales_details.find_all_count_by_date_range_and_product_group_by_price = find_all_count_by_date_range_and_product_group_by_price
sales_details.find_all_count_by_date_range_group_by_product = find_all_count_by_date_range_group_by_product
